...
def add_patient_information(user, auth_service):
    def display_patient_menu(patients):  # List of assigned patients
        ...
        for patient in patients:
            print(
                f"| {patient.get_username():<20} | {patient.get_assigned_mhwp():<15} |"
            )
        print("-" * 42)

    def display_condition_menu(
        patient,
    ):  # dislays the common conditions menu for mhwp to choose from
        clear_screen()
    ...
    while True:
        ...
        while True:
            ...
            if patient_username == "x":
                return
            patient = next(
                (p for p in assigned_patients if p.get_username() == patient_username),
                None,
            )
            if patient:
                break
        ...
        while True:
            ...
            if option == "c":
                while True:
                    ...
                    print_system_message(
                        f"Condition '{condition}' with notes saved successfully."
                    )
                    direct_to_dashboard()
                    return
                ...
                while True:
                    ...
                    if prescription_choice.isdigit() and 1 <= int(
                        prescription_choice
                    ) <= len(prescriptions):
                        ...
                        print(MHWP_BANNER_STRING)
                        print(
                            f"Adding Prescription for Patient: {patient.get_username()}"
                        )
                        prescriptions = display_prescription_menu(patient)
                        print_system_message(
                            "Invalid unit selection. Please try again."
                        )
                ...
                while True:
                    ...
                    if unit_choice.isdigit() and 1 <= int(unit_choice) <= len(
                        predefined_units
                    ):
                ...
                while True:
                    ...
                    print(f"Recorded Medication: {medication}")
                    print_system_message(
                        "Invalid dosage. Please enter a positive number."
                    )
                ...
                auth_service.save_data_to_file()
                print_system_message(
                    f"Prescription '{medication}' added and saved successfully."
                )
                direct_to_dashboard()
                return
